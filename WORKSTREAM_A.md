...
- [ ] **A1.2** Write `backend/validate_data.py` — loads every JSON in `backend/data/` via loader.py (which uses Pydantic model_validate). Prints pass/fail per file. Run after every new data file.
